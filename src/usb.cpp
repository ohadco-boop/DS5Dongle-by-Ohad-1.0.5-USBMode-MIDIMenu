...
// Defaults preserve fixed49 behavior until Windows sends a UAC value:
// speaker 100% = 0 dB, microphone level 100% = 48 dB in our UAC range.
...
float usb_speaker_volume_gain() {
    if (mute[0]) return 0.0f;
    const float db = clampf_local(volume[0], -100.0f, 0.0f);
    return powf(10.0f, db / 20.0f);
}

float usb_mic_level_scale() {
    if (mute[1]) return 0.0f;
    // The mic Feature Unit exposes 0..+48 dB to the host. Treat that as a
    // user level where 48 dB == 100% of our fixed49/base gain, so Windows can
    // attenuate the Pico mic without adding extra gain/clipping.
    return clampf_local(volume[1] / 48.0f, 0.0f, 1.0f);
}
